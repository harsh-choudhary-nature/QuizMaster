...
const HangmanGame = () => {
    const hardcodedWord = "GALAXY"; // Can be changed to a dynamic word daily if needed.
    const wordHint = "A massive system of stars, gas, and dark matter bound together by gravity";
    ...
    }, [word, maskedWord, guesses, incorrectGuesses, attemptsLeft, gameStatus, storageKey]);
        ...
        <div className={styles.gameContainer}>
            ...
            <div className={styles.gameBoard}>
                ...
                <div className={styles.alphabetGrid}>
                    {"ABCDEFGHIJKLMNOPQRSTUVWXYZ".split("").map((letter) => (
                        <button
                            key={letter}
                            className={`${styles.letterButton} ${gameStatus && guesses.includes(letter) ? (isLetterCorrect(letter) ? styles.correct : styles.incorrect) : ""
                                }`}
            ...
            {gameStatus === "won" && (
                <div className={styles.fallingRibbons}>
                    {[...Array(5)].map((_, i) => (
                        <div key={i} className={`${styles.ribbon} ${styles[`ribbon-${i % 5}`]}`} />
                    ))}
                </div>
            )}
        </div>
